...
namespace NUnit_Project.Pages.Components.Profile
{
    public class LSPage : CommonDriver
    {
        ...
        // Actions as methods
        public void languagePage(LanguageModel data)
        {
            // Click on Add new button to add Language
            driver.FindElement(addNewButton).Click();

            //Now enter Languae 
            driver.FindElement(enterLanguageName).SendKeys(data.Language);
            ...
            selectElement.SelectByText(data.Level);
        ...
        public String GetLastLanguage(String Language, String Level)
        {
            try
            {
                Thread.Sleep(3000);
            ...
            {
                // Return null if no language is found
                return null;
            }
        ...
        public void DeleteAllLanguages(LanguageModel data)
        {
            ...
            while (true)
            {
                ...
                if (deleteIconsList.Count == 0)
                    break;
                //new WebDriverWait(driver, TimeSpan.FromSeconds(20)).Until(ExpectedConditions.ElementToBeClickable(deleteIconsList[0])).Click();
        ...
        public void DeleteLanguage(LanguageModel data)
        {
            ...
            foreach (var row in rows)
            {
                ...
                if (languageCell.Text.Trim().Equals(data.Language, StringComparison.OrdinalIgnoreCase))
                {
                    Console.WriteLine($"Found Language to delete: {languageCell.Text}");
                    // Click the delete icon
                    var deleteIcon = row.FindElement(By.XPath("td[3]/span[2]/i"));
                    deleteIcon.Click();
            ...
            Console.WriteLine($"Language '{data.Language}' not found for deletion.");
        }
        public void UpdateLanguage(LanguageModel data)
        {
            ...
            foreach (var row in rows)
            {
                ...
                var languageCell = row.FindElement(By.XPath("./td[1]"));
                if (languageCell.Text.Trim().Equals(data.OriginalLanguage))
                {
                    ...
                    languageInput.Clear();
                    languageInput.SendKeys(data.Language);
